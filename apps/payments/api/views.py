...
# ✅ PLATFORM MODEL: Orchestrator now uses the Platform's global Paybill from .env
from apps.integrations.mpesa.stk_push_service import PaymentStkOrchestrator


# ================= SETTLEMENT ACCOUNTS (FORMERLY PAYMENT ACCOUNTS) =================
class PaymentAccountViewSet(viewsets.ModelViewSet):
    """
    PLATFORM SETTLEMENT MODEL:
    Manages the landlord/agency accounts where the platform will send payouts/settlements.
    Tenants do NOT pay into these accounts. Tenants pay the Platform's global Paybill.
    These accounts MUST be verified by a System Admin before the platform can send funds to them.
    """
    ...
    def perform_create(self, serializer):
        """
        Creates the settlement account and immediately initiates the verification workflow.
        The account remains is_active=False and is_verified=False until approved by Admin.
        """
        account = serializer.save(owner=self.request.user)
        # ✅ Use the service for consistent verification initiation
        PaymentVerificationService.initiate_verification(
    ...
    def request_verification(self, request, id=None):
        """Allows a landlord to re-request verification if their previous request was rejected or cancelled."""
        account = self.get_object()
        ver = PaymentVerificationService.initiate_verification(
            account_id=str(account.id),
            user_id=request.user.id
        )
        return Response({"status": "initiated", "verification_id": str(ver.id)})

    # ✅ REMOVED: The 'activate' action was removed.
    # In the Platform Model, accounts can ONLY be activated via the strict
    # PaymentVerificationService.approve_verification() workflow to prevent fraud.
...
class InvoiceViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        if role == 'admin':
            ...
            qs = qs.filter(tenancy__property__created_by=user)
        elif role in ['agency', 'manager', 'agent']:
            qs = qs.filter(tenancy__property__current_manager=user)
        else:
            qs = qs.none()
...
class PaymentHistoryViewSet(viewsets.ReadOnlyModelViewSet):
    ...
    def get_queryset(self):
        ...
        qs = Payment.objects.select_related(
            "payer", "tenancy"
        ).prefetch_related("allocations__invoice__tenancy__property")
        ...
        if role == 'admin':
            ...
            qs = qs.filter(tenancy__property__created_by=user).distinct()
        elif role in ['agency', 'manager', 'agent']:
            qs = qs.filter(tenancy__property__current_manager=user).distinct()
        else:
            qs = qs.none()
...
class FinancialActionView(viewsets.ViewSet):
    ...
    def request_stk_push(self, request):
        ...
        Triggers an STK push to the Platform's global Paybill.
        The tenant pays the platform, not the landlord directly.
        """
        ...
        if not tenancy:
            return Response(
                {"error": "Valid tenancy or invoice reference is required."},
                status=status.HTTP_400_BAD_REQUEST
            )
